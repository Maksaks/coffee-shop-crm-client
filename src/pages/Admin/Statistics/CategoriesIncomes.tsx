	...
	return (
		...
		>
			...
			<div className='grid grid-cols-12 gap-2 p-3 w-full border-b-4'>
				...
				<input
					className='col-span-3 p-1 text-[1.8vh] rounded-xl text-black'
					type='datetime-local'
					value={fromDate}
			...
			{categoriesOrdersIncomes?.length ? (
				<div className='w-[90%] bg-zinc-700/95 h-[5vh] rounded-t-2xl grid grid-cols-11 items-center px-5 py-2 font-bold text-[2vh]'>
					<label>ID</label>
					<label className='col-span-4 text-center'>TITLE</label>
					<label className='col-span-3 text-center'>COUNT OF ORDERS</label>
					<label className='col-span-3 text-center'>TOTAL INCOME</label>
				</div>
			) : (
				<></>
			)}
				...
				{categoriesOrdersIncomes?.length ? (
					categoriesOrdersIncomes.map((item, indx) => {
						return (
							<div
								key={indx}
								className={`w-[90%] h-[5vh] rounded-2xl grid grid-cols-11 items-center px-5 py-2 text-[1.8vh] text-black/80 bg-zinc-300`}
							>
								<label>{item.id}</label>
