...
const Common: FC = () => {
	...
	return (
		<motion.div
			...
			className='w-[60%] h-[1000px] mx-auto  text-white font-roboto flex flex-col gap-2 rounded-2xl bg-zinc-600 shadow-2xl items-center'
		>
				...
				{expensesAndIncomes?.length ? (
					expensesAndIncomes.map((item, indx) => {
						return (
							<div
								key={indx}
								className={`w-[90%] h-[60px] rounded-2xl grid grid-cols-10 items-center px-5 py-2 text-lg text-black/80 ${item.expenses > item.incomes ? 'bg-red-200' : 'bg-green-200'}`}
							>
								<label>{item.id}</label>
