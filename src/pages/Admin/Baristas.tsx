	...
	return (
		<>
			...
			<motion.div
				...
				className='w-[70%] h-[75vh] mt-[5vh] mx-auto text-white font-roboto flex items-center gap-5 relative'
			>
				<button
				...
				>
					<Trash2
						className={`w-[3vw] h-[5vh] p-2 rounded-full ${selectedBarista ? 'hover:stroke-black hover:bg-zinc-400' : ''}`}
					/>
				</button>
				<div className='h-[100%] w-[23%] bg-zinc-700 rounded-3xl flex items-center flex-col relative shadow-2xl'>
					<button
					...
					>
						<UserPlus className='w-[2vw] h-[3.5vh]' />
					</button>
					<h2 className='w-full p-3 border-b-4 uppercase text-center text-[2vh] font-bold'>
						Baristas
					</h2>
					<input
							...
							searchedBaristas.map((item, indx) => {
								return (
									<button
										...
										className={`p-[1vh] w-[90%] rounded-2xl hover:bg-zinc-400 text-[1.7vh] font-bold ${selectedBarista?.id == item.id ? 'bg-zinc-300 text-black underline uppercase font-bold' : 'bg-zinc-500'}`}
									>
				...
				<div className='h-[100%] w-[76%] bg-zinc-700 rounded-3xl flex flex-col shadow-2xl'>
					<form
						className='grid grid-cols-6 px-20 pt-10 h-[50%] items-center border-b-4'
						onSubmit={submitHandler}
					>
						<label className='text-[2vh] uppercase font-bold p-2 flex items-center'>
							Name:
						</label>
						<input
							required
							...
							className='w-[80%] h-[5vh] col-span-2 bg-gradient-to-r from-zinc-500 to-zinc-400 p-2 rounded-2xl hover:border-2 placeholder:text-white/70 text-[1.7vh] placeholder:text-[1.7vh]'
						...
						<input
							...
							className={`w-[80%] h-[5vh] col-span-2 bg-gradient-to-r from-zinc-500 to-zinc-400 p-2 rounded-2xl hover:border-2 placeholder:text-white/70 text-[1.7vh] placeholder:text-[1.7vh] ${email.length && selectedBarista && !isEmailValid(email) ? 'border-red-700 border-2' : ''}`}
						...
						/>
						<Button
							className='col-start-5 text-[2vh] col-span-2 h-[5vh] w-[80%] p-[0.5vh] uppercase disabled:hover:text-white disabled:hover:bg-zinc-800'
							title='Update'
							type='submit'
							disabled={!selectedBarista}
						/>
					...
					<div className='w-full h-[40%] flex flex-col items-center relative'>
						<h2 className='w-full underline underline-offset-8 text-center p-3 uppercase text-[2vh] font-bold'>
							Points
						</h2>
						<div className='w-full p-5 h-[100%] flex items-center justify-center gap-5'>
							<div className='w-[90%] h-[100%] bg-zinc-400 overflow-auto rounded-2xl p-3 flex flex-col gap-[0.5vh] items-center font-bold'>
								{selectedBarista &&
									selectedBarista.points.map((item, indx) => {
										return (
											<div
												key={indx}
												className='w-[95%] text-white bg-zinc-600 rounded-2xl grid grid-cols-12 border-2 border-white uppercase text-[1.7vh]'
											>
												<span className='p-[1vh]'>{item.id}</span>
												<span className='col-span-5 p-[1vh] border-x-2 text-center border-white'>
													{item.name}
												</span>
												<span className='p-[1vh] col-span-5 text-center border-r-2 border-white'>
													{item.address}
												</span>
												<button
												...
												>
													<Trash2 className='mx-auto w-[2vw] h-[3vh]' />
												</button>
											</div>
										)
									})}
						...
						<Button
							...
							icon={<Plus className='w-[2vw] h-[4vh]' />}
							type='button'
							disabled={!selectedBarista}
